...
paying since an earlier date; `ClearPlan` takes them off plans.

This is the one call that does not fail open. `Track` retries later and
`DecideAsync` allows, because both have a safe default; "I could not record
what this customer pays" has none, and a wrong plan is a wrong margin. So it
reports the failure to you instead of swallowing it. It still never throws.

`TrackParams` and `DecideParams` also take a `Plan`, so it can ride along with
